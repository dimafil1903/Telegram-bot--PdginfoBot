...
class GenericmessageCommand extends SystemCommand
{
    ...
    public function execute()
    {
        ...
        } else if ($text == 'Транспорт 🚍' or $text == '235'or $text == '236'or $text == '242'or $text == '243'or $text == '244'or $text == '245'or $text == '260') {
...
// Check connection
            ...
            $selec = "select  * from digest_categories  ";
            $res = mysqli_query($conn, $selec);
            $buttons=[];
            foreach ($res as $val) {
                array_push($buttons,$val['name']);
            }
            ...
            $data1 = [
                'chat_id' => $chat_id,
                'text' => "Ви знаходитесь в меню 'Послуги'",
                'reply_markup' => $keyboard,
            ];
            Request::sendMessage($data1);
...
// Check connection
        ...
        foreach ($res as $val) {
            ...
            if ($text == $val['name']) {
                ...
                $digests= "select * from digests left join digest_categories on digests.category_id=digest_categories.id where digest_categories.name='$text' ";
                $echo_text="";
                $dig = mysqli_query($conn, $digests);
                foreach ($dig as $digest) {
                    $echo_text.="\n"."*".$digest["info"]."* : "."".$digest['phone'].""."\n";
                }
                $data1 = [
                    'chat_id' => $chat_id,
                    'text' => $echo_text,
        ...
        if ($text=='Інформація про місто'){
            $selec = "select  * from info_about_cities ";
            $res = mysqli_query($conn, $selec);
            foreach ($res as $about){
                ...
                Request::sendMediaGroup([
                    'chat_id'  => $chat_id,
                    'photo_1'  =>Request::encodeFile("/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/About/Pidgorodne_gerb.png"),
                    'photo_2'  =>Request::encodeFile("/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/About/Pidhorodne_prapor.png"),
                    'photo_3'  =>Request::encodeFile("/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/About/Pidgorodne_view.png"),
                     'media'    => [
...
// Check connection
        ...
        $res = '';
        foreach ($resy as $val) {
            $select_dep = "SELECT * FROM dep_street LEFT Join street on dep_street.street_id = street.id LEFT JOIN deputy on dep_street.dep_id=deputy.id where street.name_s LIKE '%" . mb_convert_case($text,MB_CASE_TITLE, 'UTF-8') . "%' ";
            $select_dep_info = "SELECT * FROM dep_street LEFT Join street on dep_street.street_id = street.id LEFT JOIN deputy on dep_street.dep_id=deputy.id  where deputy.name LIKE '" . mb_convert_case($text,MB_CASE_TITLE, 'UTF-8') . "%'";
            $select_dep_info2 = "select * from deputy where name LIKE '" .  mb_convert_case($text,MB_CASE_TITLE, 'UTF-8') . "%'";
            ...
            if ($val['chat_id']==$chat_id and $val['message_id'] == $msg_id - 2 and $text!=='' and $val['message'] == '📄 Схема маршруту') {
                ...
                if ($text=='236' or $text=='235' or $text=='245' or $text=='260' or $text=='244' or $text=='243' or $text=='242' ) {
                    Request::sendPhoto([
                        'chat_id' => $chat_id,
                        'photo' => "/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/Upload/$text.jpg",
                    ]);
            ...
            if ($val['chat_id']==$chat_id and $val['message_id'] == $msg_id - 2 and $text!=='' and $val['message'] == '📜 Графік маршруту') {
                ...
                if ($text=='236' or $text=='235' or $text=='245' or $text=='260' or $text=='244' or $text=='243' or $text=='242' ) {
                    ...
                    Request::sendPhoto([
                        'chat_id' => $chat_id,
                        'photo' => Request::encodeFile("/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/Upload/workday_$text.jpg"),
                        'caption'=>'Будній день',
                    ]);
                    Request::sendPhoto([
                        'chat_id' => $chat_id,
                        'photo' => Request::encodeFile("/var/www/adminbot/data/www/bot.pidgorodne.info/storage/app/public/Upload/dayoff_$text.jpg"),
